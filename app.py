...
async def mass_follow_followers(target_username: str, limit: int, task_id: str = None):
    ...
    api = TikTokApi()
    try:
        await api.login_with_cookies(cookies)
            ...
            try:
                ...
                if "already following" in str(e).lower():
                    ...
                    if "ActionBlocked" in str(e):
                        return True, f"Hit a block. Followed {success}, skipped {skipped}, failed {failed}. Wait and increase delays."
...
def api_send():
    if not rate_limiter.allow():
        return jsonify({"success": False, "error": "Too many requests. Wait a minute."}), 429
    ...
    cookies = load_cookies()
    if not cookies:
        return jsonify({"success": False, "error": "No session. Add cookies."}), 401

    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        api = TikTokApi()
        loop.run_until_complete(api.login_with_cookies(cookies))
